...
class Stone:
    ...
    def __prepare_board__(self):
        ...
        self.board = np.zeros((maxV + 2, maxH + 2), dtype = object)
        return self
...
class Sand(Stone):
    ...
    def __add_grain__(self):
        ...
        if self.board[prevPos[0], prevPos[1]] == "@":
            return self
        ...
        while canMove:
            ...
            try:
                ...
                if self.board[prevPos[0] + 1, prevPos[1]] not in ["#", "@"]:
                    self.board[prevPos[0] + 1, prevPos[1]] = "@"
                    self.board[prevPos[0], prevPos[1]] = "."
                    prevPos = [prevPos[0] + 1, prevPos[1]]
                elif self.board[prevPos[0] + 1, prevPos[1] - 1] not in ["#", "@"]:
                    self.board[prevPos[0] + 1, prevPos[1] - 1] = "@"
                    self.board[prevPos[0], prevPos[1]] = "."
...
def run():
    ...
    # for line in sand.board_ready().board:
        # print(line[494 : 504])
    grainNo = sand.board_ready().simulate_sand().grainCounter
    print()
    return grainNo
print(run())
